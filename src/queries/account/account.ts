...
export default function useGetAccountQuery(id: string) {
  return useQuery(['accountQuery'], () => AccountApi.getAccount(id), {
    ...
    onSuccess: (data) => {
      data.broker_id = BROKER_ID[data.broker_id];
      data.status = ACCOUNT_STATUS[data.status];
      data.assets = Number(data.assets).toLocaleString();
      data.payments = Number(data.payments).toLocaleString();
      data.number = maskingFunc(data.number);
      data.is_active = data.is_active ? '활성화' : '비활성화';
    },
    enabled: !!id,
  });
}
